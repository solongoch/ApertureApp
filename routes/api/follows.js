    ...
    User.findById(req.params.user_id)
      .then(user => {
        if (user) {
            ...
            .then(result => {
              User.findOne({ email: req.user.email }).then(user => {
                if (
                  ...
                  return res
                    .status(400)
                    .json({ alreadyfollow: "You already following the user" });
                }
          ...
          return res
            .status(404)
            .json({ success: false, message: "There is no such profile" });
        }
      })
      .catch(err => {
...
router.put(
  ...
  (req, res) => {
    ...
    if (req.user.id === req.params.user_id) {
      return res
        .status(400)
        .json({ alreadyfollow: "You cannot unfollow follow yourself" });
    }
    User.findById(req.params.user_id)
      .then(user => {
        if (user) {
          ...
          if (
            ...
            user.save().then(data => {
              User.findOne({ email: req.user.email }).then(doc => {
                //check if unfollow user in following [] then remove
                if (
                  ...
                  return res.status(400).json({
                    message: "You are not the following the user yet"
                  });
                }
              });
            });
            ...
            return res
              .status(400)
              .json({ message: "You are not the following the user yet" });
          }
        } else {
          //no user founf to follow
          return res
            .status(404)
            .json({ success: false, message: "There is no such profile" });
        }
